...
if [ $dom = CAS -o $dom = SAM -o $dom = AUS ]; then
...
## ERA 
nlist=$ndir/${dom~~}_era5_masked.list
get_records_data $nlist 
...
## MERRA
nlist=$ndir/${dom~~}_merra_masked.list
get_records_data $nlist 
...
## JRA
nlist=$ndir/${dom~~}_jra55_masked.list
get_records_data $nlist 
...
ncl -Q $args tools/plot_frq_reanalysis_masked.ncl
